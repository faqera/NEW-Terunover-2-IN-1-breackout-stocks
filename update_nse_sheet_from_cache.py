...
2. INDstocks से हर स्टॉक का live LTP लाता है (Retry Logic के साथ)
...
LTP_MAX_WORKERS = 3  # parallel LTP calls (balance: speed vs rate-limit)
...
def get_indstocks_headers():
    """हर run में एक बार नया token लेकर headers बना देता है"""
...
def build_security_id_map(headers):
    if os.path.exists(SECURITY_ID_CACHE_FILE):
        ...
        if age < 20 * 3600:  # 20 घंटे तक cache reuse
...
# ============================================================
# STEP 0.5: Live LTP लाना (with Retry Logic)
# ============================================================
...
def fetch_one_ltp_with_delay(headers, symbol, exchange, security_id, delay=0.3):
    """Retry के लिए - पहले delay, फिर API call"""
    time.sleep(delay)
    return fetch_one_ltp(headers, symbol, exchange, security_id)
...
def fetch_live_ltp_all(symbols, security_id_map, headers):
    ...
    # --- दूसरा राउंड (धीमा, सिर्फ फेल हुए स्टॉक्स) ---
    if failed_symbols:
